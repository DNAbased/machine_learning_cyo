...
```{r validation_check}
glm_performance = confusionMatrix(predict(fit_glm, validation_data_x), validation_data_y)$byClass
glm_performance = as.data.frame(glm_performance)
names(glm_performance) = c("Value")
kable(glm_performance)
```

While the accuracy of the model is close to 0.8, the very high sensitivity close to 1 in combination with the rather low specificity indicates that predicting 'affected' still occurs very frequently, regardless of the actual outcome.
...
More importantly, the accuracy on the `validation` data showed a slight increase. Simultaneously, the specificity dropped from 0.27 to below 0.2. 
